...
from shared.event_config import (
    EXCHANGE, EXCHANGE_TYPE, ROUTE_QUEUE, ROUTING_KEY_LOCATION_UPDATED,
    DLX, DLQ, RETRY_EXCHANGE, RETRY_TTL_MS, MAX_RETRIES, RETRY_HEADER,
    EVENT_TYPE_LOCATION_FAILED, ROUTING_KEY_LOCATION_FAILED,
)
from shared.metrics import events_consumed_total, events_failed_total
from shared.logging_config import setup_logging

logger = setup_logging(config.SERVICE_NAME)

RETRY_QUEUE = ROUTE_QUEUE + ".retry"
    ...
    try:
        ...
        if retries >= MAX_RETRIES:
            ...
            events_failed_total.labels(
                service=config.SERVICE_NAME, queue=ROUTE_QUEUE
            ).inc()
        ...
        events_consumed_total.labels(
            service=config.SERVICE_NAME, queue=ROUTE_QUEUE, status="success"
        ).inc()
        ...
        events_consumed_total.labels(
            service=config.SERVICE_NAME, queue=ROUTE_QUEUE, status="failed"
        ).inc()
...
def start_consumer():
    ...
    while True:
        try:
            ...
            channel.queue_bind(queue=RETRY_QUEUE, exchange=RETRY_EXCHANGE, routing_key=ROUTE_QUEUE)

            channel.queue_declare(queue=ROUTE_QUEUE, durable=True, arguments={
                "x-dead-letter-exchange": DLX,
            })
            channel.queue_bind(queue=ROUTE_QUEUE, exchange=EXCHANGE, routing_key=ROUTING_KEY_LOCATION_UPDATED)

            channel.basic_consume(queue=ROUTE_QUEUE, on_message_callback=callback, auto_ack=False)
